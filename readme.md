...
- RAG AI Assistant: a chat assistant (Groq/Llama 3.3, with an automatic fallback to a second model if the first hits its rate limit) whose answers are grounded in this platform's own real data -- keyword-overlap retrieval over the curated catalogue pulls in the real climate, live budget, live Wikipedia popularity, real climate-derived Best Months and real traveler reviews for the destinations actually relevant to the question, and the model is instructed to answer only from that context and say so honestly when the real data doesn't cover what's asked, rather than inventing facts. For places outside the 23-destination curated catalogue (most real places), it falls back to a real live Geoapify search instead of claiming no data exists -- including a real live temple/place-of-worship search when the question asks for one. Chats naturally for greetings/small talk rather than forcing every reply back onto destination data. Chat history is saved per user (or guest) and clearable on demand
- AI Trip Planner: a real day-by-day itinerary generator built from this platform's own real nearby restaurants, live hotels and lifestyle spots for the chosen destination -- any real place, not just the curated catalogue. Runs on Gemini (a separate provider/quota from the RAG Assistant's Groq usage, so heavy use of one can't starve the other), with the same Groq two-model chain as a fallback if Gemini is unavailable. Never invents a place name; honestly says so when there isn't enough real data to fill a day, and gracefully reuses real favorites (framed as a real traveler revisiting a place) for longer trips rather than padding with filler
